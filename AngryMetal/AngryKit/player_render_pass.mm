...
PlayerRenderPass::PlayerRenderPass(id<MTLDevice> device, id<MTLLibrary> library)
    : RenderPass(RenderPassType::player)
{
    _attributes = {
        render::AttributeType::model_matrix,
        render::AttributeType::projection_view_matrix,
        render::AttributeType::view_position,
        render::AttributeType::aim_rotation,
    ...
    _fragment_uniforms.direction_light.direction = simd_normalize(simd_float3{-1.0f, -1.0f, -1.0f});
    _fragment_uniforms.direction_light.color = _light_color;

    _fragment_uniforms.ambient_color = _ambient_color;
}

void PlayerRenderPass::set_attribute(const render::AttributeVariant& attribute)
{
    ...
    else if (const auto& a = std::get_if<render::AimRotationMatrixAttribute>(&attribute))
    {
        _vertex_uniforms.aim_rotation = a->value;
    }
...
void PlayerRenderPass::encode(id<MTLRenderCommandEncoder> command_encoder)
{
    [command_encoder setRenderPipelineState:_render_state.get()];

    [command_encoder setVertexBytes:&_vertex_uniforms length:sizeof(Uniforms) atIndex:3];
    [command_encoder setFragmentBytes:&_fragment_uniforms length:sizeof(CommonFragmentUniforms) atIndex:0];
}

void PlayerRenderPass::setup_state(id<MTLDevice> device, id<MTLLibrary> library)
{
    ...
    objc::Ref<id<MTLFunction>> vertex_function([library newFunctionWithName:@"vertex_shader"]);
    if (!vertex_function)
    {
        throw std::runtime_error("vertex function");
    }

    objc::Ref<id<MTLFunction>> fragment_function([library newFunctionWithName:@"player_fragment_shader"]);
